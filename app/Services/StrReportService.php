...
class StrReportService
{
    ...
    /**
     * Convert a StrDraft to a formal StrReport.
     * Used by StrStudioController::convert()
     */
    public function convertToStrReport(StrDraft $draft): StrReport
    {
        ...
        $strReport = StrReport::create([
            ...
            'transaction_ids' => $draft->transaction_ids ?? [],
            'reason' => $draft->narrative,
            'supporting_documents' => [],
            'status' => StrStatus::Draft,
            'created_by' => $draft->created_by,
            'suspicion_date' => $draft->created_at,
            'filing_deadline' => $draft->filing_deadline,
        ]);

        $draft->update(['converted_to_str_id' => $strReport->id]);

        Log::info('StrReportService: Converted StrDraft to StrReport', [
            'str_draft_id' => $draft->id,
            'str_report_id' => $strReport->id,
        ]);
    ...
    /**
     * Get filing deadline summary including StrDraft deadlines.
     * Merges StrReport deadlines with StrDraft deadlines for complete view.
     */
    public function getCombinedFilingDeadlineSummary(): array
    {
        $strReportSummary = $this->getFilingDeadlineSummary();

        $draftUpcomings = \App\Models\StrDraft::pending()
            ->whereNotNull('filing_deadline')
            ->where('filing_deadline', '>=', now())
            ->orderBy('filing_deadline', 'asc')
            ->get();

        $draftOverdue = \App\Models\StrDraft::pending()
            ->whereNotNull('filing_deadline')
            ->where('filing_deadline', '<', now())
            ->orderBy('filing_deadline', 'asc')
            ->get();
        ...
        return [
            'upcoming_count' => $strReportSummary['upcoming_count'] + $draftUpcomings->count(),
            'overdue_count' => $strReportSummary['overdue_count'] + $draftOverdue->count(),
            'upcoming_reports' => $strReportSummary['upcoming_reports'],
            'upcoming_drafts' => $draftUpcomings->values(),
            'overdue_reports' => $strReportSummary['overdue_reports'],
            'overdue_drafts' => $draftOverdue->values(),
            'next_deadline' => $strReportSummary['next_deadline'],
            'urgent_count' => $strReportSummary['urgent_count'],
        ];
    }
}
