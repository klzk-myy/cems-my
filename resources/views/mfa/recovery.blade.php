    ...
    <div class="flex min-h-screen">
        <main class="flex-1 bg-[#f7f7f8] p-8 overflow-y-auto flex items-center justify-center">
            <div class="bg-white border border-[#e5e5e5] rounded-xl p-8 max-w-md w-full">
                <h1 class="text-2xl font-semibold text-[#171717] mb-2">Account Recovery</h1>
                <p class="text-sm text-[#6b6b6b] mb-6">Use a recovery code to access your account</p>
                <form method="POST" action="{{ route('mfa.recovery') }}">
                    @csrf
                    <div class="mb-4">
                        <input type="text" name="code" placeholder="Recovery Code" class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg" required>
                    </div>
