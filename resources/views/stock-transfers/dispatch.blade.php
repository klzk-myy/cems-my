...
@section('title', 'Dispatch Stock Transfer')

@section('content')
<div class="card">
    <div class="px-6 py-4 border-b border-[--color-border]">
        <h3 class="text-base font-semibold text-[--color-ink]">Dispatch Stock Transfer</h3>
    </div>
    <div class="p-6">
        @if(isset($transfer))
        <div class="mb-6">
            <h1 class="text-2xl font-semibold text-[--color-ink] mb-2">Dispatch Stock Transfer</h1>
                ...
                <div>
                    ...
                    <dd>
                        <span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded bg-blue-100 text-blue-700">
                            {{ $transfer->status ?? 'Ready for Dispatch' }}
                        </span>
                    </dd>
                </div>
                <div>
                    <dt class="text-sm text-[--color-ink-muted]">Approved By</dt>
                    <dd class="font-medium">{{ $transfer->hqApprover->username ?? 'N/A' }}</dd>
                </div>
            </dl>
        </div>

        @if(isset($items) && $items->count() > 0)
        <h4 class="text-sm font-medium text-[--color-ink] mb-3">Items to Dispatch</h4>
        ...
        <form method="POST" action="{{ route('stock-transfers.dispatch', $transfer->id ?? 0) }}">
            @csrf
            <div class="mb-4">
                <label class="block text-sm font-medium text-[--color-ink] mb-1">Dispatch Note</label>
                <textarea name="note" class="w-full px-4 py-2.5 text-sm bg-white border border-[--color-border] rounded-lg" rows="2" placeholder="Optional dispatch notes"></textarea>
            </div>
