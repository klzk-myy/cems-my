...
@section('title', 'Approve Stock Transfer (HQ)')

@section('content')
<div class="card">
    <div class="px-6 py-4 border-b border-[--color-border]">
        <h3 class="text-base font-semibold text-[--color-ink]">HQ Approval</h3>
    </div>
    <div class="p-6">
        @if(isset($transfer))
        <div class="mb-6">
            <h1 class="text-2xl font-semibold text-[--color-ink] mb-2">HQ Approval - Stock Transfer</h1>
                ...
                <div>
                    ...
                    <dd>
                        <span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded bg-yellow-100 text-yellow-700">
                            {{ $transfer->status ?? 'Pending HQ Approval' }}
                        </span>
                    </dd>
                </div>
                <div>
                    <dt class="text-sm text-[--color-ink-muted]">Approved By BM</dt>
                    <dd class="font-medium">{{ $transfer->bmApprover->username ?? 'Pending' }}</dd>
                </div>
            </dl>
        </div>
        ...
        <form method="POST" action="{{ route('stock-transfers.approve-hq', $transfer->id ?? 0) }}">
            @csrf
            <div class="mb-4">
                <label class="block text-sm font-medium text-[--color-ink] mb-1">HQ Approval Note</label>
                <textarea name="note" class="w-full px-4 py-2.5 text-sm bg-white border border-[--color-border] rounded-lg" rows="2" placeholder="Note for HQ approval"></textarea>
            </div>
