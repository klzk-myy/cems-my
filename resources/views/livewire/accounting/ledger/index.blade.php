<div>
    {{-- Header --}}
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-[--color-ink]">Chart of Accounts</h1>
        <p class="text-sm text-[--color-ink-muted]">Ledger accounts grouped by type</p>
    </div>
        ...
        @if(!empty($accountsByType[$accountType]))
            <div class="card mb-6">
                <div class="card-header">
                    <h3 class="card-title text-[--color-ink]">{{ $accountType }}</h3>
                </div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-[--color-ink-muted]">{{ $account['account_class'] ?? 'N/A' }}</td>
                                <td class="text-right">
    ...
    @if(empty($accountsByType['Asset']) && empty($accountsByType['Liability']) && empty($accountsByType['Equity']) && empty($accountsByType['Revenue']) && empty($accountsByType['Expense']))
        <div class="card">
            <div class="card-body text-center py-12">
                <p class="text-[--color-ink-muted]">No chart of accounts found</p>
            </div>
        </div>
    @endif
</div>
