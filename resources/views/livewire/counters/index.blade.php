...
<div class="flex items-center gap-3">
    @if(auth()->user()->role->isTeller())
        @php $openCounter = null; @endphp
        @foreach($counters as $c)
            @if($c['status'] === 'Open' && $c['operator_name'] === auth()->user()->username)
                @php $openCounter = $c; @endphp
            @endif
        @endforeach
        @if($openCounter)
            <a href="{{ route('counters.close.show', $openCounter['id']) }}" class="btn btn-danger">
                Close Counter
            </a>
        @else
            <a href="{{ route('counters.open') }}" class="btn btn-primary">
                Open Counter
            </a>
        @endif
    @endif
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <div class="card">
        <div class="card-header">
            ...
            @php
                $statusClass = match($counter['status']) {
                    'Open' => 'badge-success',
            ...
            @endphp
            <span class="badge {{ $statusClass }}">{{ $counter['status_label'] }}</span>
        </div>
        <div class="card-body">
            <div class="space-y-3">
        ...
        <div class="card-footer flex gap-2">
            <a href="{{ route('counters.index') }}" class="btn btn-ghost btn-sm flex-1">View</a>
            @if($counter['status'] === 'Open' && auth()->user()->role->isTeller())
                <a href="{{ route('counters.handover.show', $counter['id']) }}" class="btn btn-secondary btn-sm flex-1">Handover</a>
            @endif
        </div>
    </div>
    @empty
