<div>
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-xl font-semibold text-gray-900">Cancel Transaction #{{ $transaction->id }}</h1>
            <p class="text-sm text-gray-500">Manager approval required for cancellation</p>
        </div>
    </div>

    <div class="max-w-4xl mx-auto">
        {{-- Status Banner --}}
        <div class="card mb-6 border-2 border-amber-500">
            <div class="card-body">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-semibold text-amber-500">Pending Cancellation</p>
                        <p class="text-sm text-gray-500">This transaction is awaiting manager approval to be cancelled</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card mb-6">
            ...
            <div class="card-body">
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        @if($this->requestedByUser)
                            <p class="text-sm text-gray-500">{{ $this->requestedByUser->role->label() }}</p>
                        @endif
                    </div>
                    <div>
                ...
                <div class="mt-4">
                    <p class="text-sm text-gray-500 mb-1">Reason for Cancellation</p>
                    <div class="p-4 bg-gray-50 rounded-lg">
                        <p class="text-gray-900">{{ $this->cancellationDetails['reason'] }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card mb-6">
            ...
            <div class="card-body">
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        <p class="text-sm text-gray-500 mb-1">Teller</p>
        ...
        @if(!$showConfirmation)
            <div class="card">
                <div class="card-body">
                    <p class="text-sm text-gray-500 mb-4">Please review the cancellation request and choose an action:</p>
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg mb-4">
                        <div class="flex items-center gap-3">
                            <svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                            <span class="text-sm text-gray-500">Approving will cancel the transaction. Rejecting will return it to its previous status.</span>
                        </div>
                    </div>
                </div>
                ...
                <form wire:submit="processApproval" id="processApproval" class="card-body">
                    <div class="flex items-center gap-3 mb-4">
                        @if($action === 'approve')
                            ...
                            <div>
                                <p class="font-semibold text-green-600">Confirm Cancellation Approval</p>
                        ...
                        @else
                            ...
                            <div>
                                <p class="font-semibold text-amber-500">Confirm Cancellation Rejection</p>
                                <p class="text-sm text-gray-500">Transaction will be returned to its previous status</p>
                            </div>
                        @endif
                    </div>
