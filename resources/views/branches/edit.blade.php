...
<head>
    ...
    <title>Edit Branch - CEMS-MY</title>
...
<body class="font-sans antialiased">
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 bg-[#f7f7f8] p-8 overflow-y-auto">
            <div class="mb-6">
                <h1 class="text-2xl font-semibold text-[#171717]">Edit Branch</h1>
                <p class="text-sm text-[#6b6b6b] mt-1">Update branch information</p>
            </div>
            <form method="POST" action="{{ route('branches.update', $branch) }}" class="bg-white border border-[#e5e5e5] rounded-xl p-6">
                @csrf @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-[#171717] mb-2">Branch Code</label>
                        <input type="text" name="code" value="{{ $branch->code }}" class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-[#171717] mb-2">Branch Name</label>
                        <input type="text" name="name" value="{{ $branch->name }}" class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg" required>
                    </div>
                    <div>
                        ...
                        <select name="type" class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg" required>
                            ...
                            <option value="{{ $value }}" {{ $branch->type === $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="parent_id" class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg">
                            ...
                            <option value="{{ $parent->id }}" {{ $branch->parent_id === $parent->id ? 'selected' : '' }}>{{ $parent->code }} - {{ $parent->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-[#171717] mb-2">Address</label>
                        <input type="text" name="address" value="{{ $branch->address }}" class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-[#171717] mb-2">City</label>
                        <input type="text" name="city" value="{{ $branch->city }}" class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-[#171717] mb-2">State</label>
                        <input type="text" name="state" value="{{ $branch->state }}" class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-[#171717] mb-2">Phone</label>
                        <input type="text" name="phone" value="{{ $branch->phone }}" class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-[#171717] mb-2">Email</label>
                        <input type="email" name="email" value="{{ $branch->email }}" class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg">
                    </div>
                    <div class="md:col-span-2 flex items-center gap-4">
                        <label class="flex items-center gap-2">
                            <input type="checkbox" name="is_active" value="1" {{ $branch->is_active ? 'checked' : '' }} class="w-4 h-4 rounded border-[#e5e5e5]">
                            <span class="text-sm text-[#171717]">Active</span>
                        </label>
                        <label class="flex items-center gap-2">
                            <input type="checkbox" name="is_main" value="1" {{ $branch->is_main ? 'checked' : '' }} class="w-4 h-4 rounded border-[#e5e5e5]">
                            <span class="text-sm text-[#171717]">Main Branch</span>
                        </label>
                    </div>
                </div>
                <div class="mt-6 flex gap-3">
                    <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-[#0a0a0a] rounded-lg hover:bg-[#262626]">Update Branch</button>
                    <a href="{{ route('branches.show', $branch) }}" class="px-4 py-2 text-sm font-medium text-[#171717] bg-white border border-[#e5e5e5] rounded-lg hover:bg-[#f7f7f8]">Cancel</a>
                </div>
            </form>
        </main>
    </div>
