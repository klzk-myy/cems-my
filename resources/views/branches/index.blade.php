    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 bg-[#f7f7f8] p-8 overflow-y-auto">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-semibold text-[#171717]">Branches</h1>
                    <p class="text-sm text-[#6b6b6b] mt-1">Manage branch locations and settings</p>
                </div>
                <a href="{{ route('branches.create') }}" class="px-4 py-2 text-sm font-medium text-white bg-[#0a0a0a] rounded-lg hover:bg-[#262626]">Add Branch</a>
            </div>
            <div class="bg-white border border-[#e5e5e5] rounded-xl overflow-hidden">
                <table class="w-full text-sm">
                    <thead class="bg-[#f7f7f8] border-b border-[#e5e5e5]">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#6b6b6b]">Code</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-[#e5e5e5] hover:bg-[#f7f7f8]/50">
                            <td class="px-4 py-3 font-mono text-xs text-[#171717]">{{ $branch->code }}</td>
                            <td class="px-4 py-3 text-[#171717]">{{ $branch->name }}</td>
                            <td class="px-4 py-3 text-[#6b6b6b]">{{ ucfirst(str_replace('_', ' ', $branch->type)) }}</td>
                            <td class="px-4 py-3">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center text-[#6b6b6b]">No branches found</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
