...
@section('header-title')
    <h1 class="text-xl font-semibold text-[--color-ink]">Edit STR-2026-001</h1>
@endsection

@section('header-actions')
    <a href="/str/1" class="btn btn-ghost">
        Cancel
    </a>
@endsection
...
<form method="POST" action="{{ route('str.update', 1) }}" class="space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white rounded-xl border border-[--color-border] p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="date" name="transaction_date" value="2026-05-03" class="w-full px-4 py-2 border border-[--color-border] rounded-lg focus:outline-none focus:ring-2 focus:ring-[--color-accent]/30" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-[--color-ink] mb-2">Transaction Amount (MYR)</label>
                <input type="number" name="amount" step="0.01" value="85000.00" class="w-full px-4 py-2 border border-[--color-border] rounded-lg focus:outline-none focus:ring-2 focus:ring-[--color-accent]/30" required>
            </div>
            ...
            <div>
                ...
                <select name="transaction_type" class="w-full px-4 py-2 border border-[--color-border] rounded-lg focus:outline-none focus:ring-2 focus:ring-[--color-accent]/30" required>
                    <option value="cash_deposit" selected>Cash Deposit</option>
    ...
    <div class="bg-white rounded-xl border border-[--color-border] p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="customer_name" value="Ahmad bin Hassan" class="w-full px-4 py-2 border border-[--color-border] rounded-lg focus:outline-none focus:ring-2 focus:ring-[--color-accent]/30" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-[--color-ink] mb-2">IC/Passport Number</label>
                <input type="text" name="customer_ic" value="701225-10-1234" class="w-full px-4 py-2 border border-[--color-border] rounded-lg focus:outline-none focus:ring-2 focus:ring-[--color-accent]/30">
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-[--color-ink] mb-2">Customer Address</label>
                <textarea name="customer_address" rows="3" class="w-full px-4 py-2 border border-[--color-border] rounded-lg focus:outline-none focus:ring-2 focus:ring-[--color-accent]/30">No. 123, Jalan Ampang, 55000 Kuala Lumpur</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-[--color-border] p-6">
        ...
        <div class="space-y-6">
            <div>
                ...
                <textarea name="description" rows="4" class="w-full px-4 py-2 border border-[--color-border] rounded-lg focus:outline-none focus:ring-2 focus:ring-[--color-accent]/30" required>Customer made multiple cash deposits totaling MYR 85,000 within a short period. The transactions appear inconsistent with the customer's known profile and business activities.</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-[--color-ink] mb-2">Reasons for Suspicion</label>
                <textarea name="reasons" rows="3" class="w-full px-4 py-2 border border-[--color-border] rounded-lg focus:outline-none focus:ring-2 focus:ring-[--color-accent]/30" required>Structuring behavior - multiple deposits just below reporting threshold
Inconsistent customer profile
Unusual transaction pattern for this customer</textarea>
            </div>
    ...
    <div class="flex items-center justify-end gap-4">
        <a href="/str/1" class="px-6 py-2 text-sm font-medium text-[--color-ink-muted] hover:text-[--color-ink] transition-colors">
            Cancel
        </a>
