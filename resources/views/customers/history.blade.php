...
<div class="card">
    <div class="card-header">
        ...
        <div class="text-sm text-[--color-ink-muted]">
            {{ $customer->name ?? 'N/A' }} - {{ $customer->ic_number ?? 'N/A' }}
        </div>
    </div>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="font-mono text-right">{{ number_format($tx['rate'] ?? 0, 4) }}</td>
                    <td class="font-mono text-right">{{ number_format($tx['myr_value'] ?? 0, 2) }}</td>
                    <td>
                        @if(isset($tx['status']))
                ...
                @empty
                <tr><td colspan="7" class="text-center py-8 text-[--color-ink-muted]">No transactions found</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
