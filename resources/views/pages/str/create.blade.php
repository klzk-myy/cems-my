<x-app-layout title="New STR Report">
    <div class="p-6">
        ...
        <form method="POST" action="{{ route('str.store') }}" class="bg-white rounded-lg shadow p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="filing_deadline" class="w-full border rounded px-3 py-2">
                </div>
            </div>

            <div class="mt-6 flex gap-4">
                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Create STR</button>
                <a href="{{ route('str.index') }}" class="px-6 py-2 border rounded hover:bg-gray-50">Cancel</a>
            </div>
        </form>
    </div>
</x-app-layout>