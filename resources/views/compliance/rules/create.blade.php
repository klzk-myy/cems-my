...
@section('title', 'Create AML Rule - CEMS-MY')

@section('content')
<div class="mb-6">
    <h2 class="text-xl font-semibold text-gray-800 mb-1">Create AML Rule</h2>
    <p class="text-gray-500 text-sm">Configure a new Anti-Money Laundering rule for transaction monitoring</p>
</div>
...
<form action="{{ route('compliance.rules.store') }}" method="POST" class="bg-white rounded-lg shadow-sm p-6">
    @csrf
    ...
    <div class="mb-6">
        ...
        <input type="text" id="rule_code" name="rule_code"
            ...
            value="{{ old('rule_code') }}" placeholder="e.g., VEL-001"
    ...
    <div class="mb-6">
        ...
        <input type="text" id="rule_name" name="rule_name"
            class="w-full p-3 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
            value="{{ old('rule_name') }}" placeholder="e.g., High Velocity Alert"
    ...
    <div class="mb-6">
        ...
        <textarea id="description" name="description"
            ...
            placeholder="Describe what this rule detects and why...">{{ old('description') }}</textarea>
    </div>
    ...
    <div class="mb-6">
        ...
        <select id="action" name="action"
            ...
            required>
            <option value="flag" {{ old('action') == 'flag' ? 'selected' : '' }}>Flag - Create alert for review</option>
            <option value="hold" {{ old('action') == 'hold' ? 'selected' : '' }}>Hold - Put transaction on hold</option>
            <option value="block" {{ old('action') == 'block' ? 'selected' : '' }}>Block - Reject the transaction</option>
        </select>
    ...
    <div class="mb-6">
        ...
        <input type="number" id="risk_score" name="risk_score"
            class="w-full p-3 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
            value="{{ old('risk_score', 0) }}" min="0" max="100" required>
    ...
    <div class="mb-6">
        ...
        <input type="hidden" name="conditions" id="conditions-json" value="{{ old('conditions', '{}') }}">
    </div>

    <div class="flex gap-4 pt-4 mt-6 border-t border-gray-200">
        <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded font-semibold hover:bg-blue-700 transition-colors">Create Rule</button>
        <a href="{{ route('compliance.rules.index') }}" class="px-6 py-2 bg-gray-200 text-gray-700 no-underline rounded font-semibold hover:bg-gray-300 transition-colors">Cancel</a>
    </div>
</form>
...
function updateConditionSchema(ruleType) {
    ...
    const defaults = schema.default_conditions || {};
            ...
            html += `
                <div>
                    ...
                    <input type="number" class="w-full p-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500" id="cond_window_hours"
                        value="${defaults.window_hours || 24}" min="1"
                ...
                <div>
                    ...
                    <input type="number" class="w-full p-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500" id="cond_max_transactions"
                        value="${defaults.max_transactions || 10}" min="1"
                        onchange="updateConditionsPreview()">
                </div>
                <div>
                    ...
                    <input type="number" class="w-full p-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500" id="cond_cumulative_threshold"
                        value="${defaults.cumulative_threshold || ''}" placeholder="Optional"
                        onchange="updateConditionsPreview()">
                </div>
            `;
            break;
            ...
            html += `
                <div>
                    ...
                    <input type="number" class="w-full p-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500" id="cond_window_days"
                        value="${defaults.window_days || 1}" min="1"
                        onchange="updateConditionsPreview()">
                </div>
                <div>
                    ...
                    <input type="number" class="w-full p-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500" id="cond_min_transaction_count"
                        value="${defaults.min_transaction_count || 3}" min="2"
                        onchange="updateConditionsPreview()">
                </div>
                <div>
                    ...
                    <input type="number" class="w-full p-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500" id="cond_aggregate_threshold"
                        value="${defaults.aggregate_threshold || 50000}" min="1"
                        onchange="updateConditionsPreview()">
                </div>
            `;
            break;
            ...
            html += `
                <div>
                    ...
                    <input type="number" class="w-full p-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500" id="cond_min_amount"
                        value="${defaults.min_amount || 50000}" min="1"
                        onchange="updateConditionsPreview()">
                </div>
                <div>
                    ...
                    <select class="w-full p-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500 cursor-pointer" id="cond_currency" onchange="updateConditionsPreview()">
                        <option value="MYR" ${defaults.currency === 'MYR' ? 'selected' : ''}>MYR</option>
                        ...
                        <option value="SGD" ${defaults.currency === 'SGD' ? 'selected' : ''}>SGD</option>
                    </select>
                </div>
            `;
            break;
        ...
        case 'frequency':
            html += `
                <div>
                    <label class="block mb-1 text-xs font-medium text-gray-600">Window (hours)</label>
                    <input type="number" class="w-full p-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500" id="cond_window_hours"
                        value="${defaults.window_hours || 1}" min="1"
                        onchange="updateConditionsPreview()">
                </div>
                <div>
                    <label class="block mb-1 text-xs font-medium text-gray-600">Max Transactions</label>
            ...
            html += `
                ...
                <div>
                    ...
                    <select class="w-full p-2 border border-gray-200 rounded text-sm focus:outline-none focus:border-blue-500 cursor-pointer" id="cond_match_field" onchange="updateConditionsPreview()">
                        <option value="customer_nationality" ${defaults.match_field === 'customer_nationality' ? 'selected' : ''}>
...
document.addEventListener('DOMContentLoaded', function() {
    const selectedType = document.querySelector('input[name="rule_type"]:checked');
    if (selectedType) {
        updateConditionSchema(selectedType.value);
    }
});
</script>
@endsection
